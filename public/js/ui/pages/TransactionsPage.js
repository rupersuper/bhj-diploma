...
class TransactionsPage {
  ...
  registerEvents() {
    ...
    Array.from(removeTransactionBtns).forEach((item) => {
  ...
  removeAccount() {
    if (!this.waitingForResponse) {
      let windowConfirm = confirm("Вы действительно хотите удалить этот счет?");

      if (!windowConfirm || !this.lastOptions) {
        return;
      }
    }
  ...
  removeTransaction(id) {
    let windowConfirm = confirm("Вы действительно хотите удалить эту транзакцию?");
  ...
  renderTransactions(data) {
    ...
    content.innerHTML = allTransactionHTML;
  }
}
